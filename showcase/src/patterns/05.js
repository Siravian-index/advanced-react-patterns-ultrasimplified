...
const MediumClap = ({ children, onClap, values = null, style: userStyles = {}, className }) => {
  ...
  const isControlled = Boolean(values) && onClap
  ...
  // checks if component is controlled
  const getState = useCallback(() => isControlled ? values : clapState, [isControlled, values, clapState])
  const memoizedValue = useMemo(() => ({ ...getState(), setRef }), [setRef, getState])

  const classNames = [styles.clap, className].join(' ').trim()
  return (
    <Provider value={memoizedValue}>
      <button ref={setRef} data-keyref='clapRef' className={classNames} onClick={handleClapClick} style={userStyles}>
        {children}
      </button>
    </Provider>
  )
...
const ClapCount = ({ style: userStyles = {}, className }) => {
  ...
  return (
    <span ref={setRef} data-keyref='clapCountRef' className={classNames} style={userStyles}>
      +{count}
    </span>
  )
}
...
const CountTotal = ({ style: userStyles = {}, className }) => {
  ...
  return (
    <span ref={setRef} data-keyref='clapTotalRef' className={classNames} style={userStyles}>
      {countTotal}
    </span>
  )
}
...
const Usage = () => {
  ...
  return (
    <div style={{}}>
      <MediumClap values={state} onClap={handleClap} className={userStyles.clap}>
